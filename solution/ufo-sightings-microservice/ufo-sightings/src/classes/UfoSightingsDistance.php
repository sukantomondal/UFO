...
class UfoSightingsDistance {
	...
	$sql = "SELECT * FROM ufo_sightings_bookeeper WHERE city IS NOT NULL AND city <> '' AND state IS NOT NULL AND state <> '' AND country IS NOT NULL AND country <> '' AND shape IS NOT NULL AND shape <> '' AND duration_seconds <> 0 AND duration_text <> '' AND description IS NOT NULL AND description <> '' AND reported_on IS NOT NULL AND latitude IS NOT NULL AND longitude IS NOT NULL LIMIT {$offset},{$limit}";
